...
class IconControl extends Component {
	...
	renderIcon = ( option ) => {
		return <span><i class={ option.value }></i> { option.label }</span>;
	}
	...
	render() {
		...
		const id = `inspector-mb-icon-control-${ instanceId }`;

		const that = this;

		const onChangeValue = ( selectedOption ) => {
			that.setState( { selectedOption: selectedOption.value } );

			onChange( selectedOption.value );
		}

		/*
			How to get icon list:

			Go to fontawesome cheatsheet https://fontawesome.com/cheatsheet and then add the following in the console
		...
		*/
		...
		Object.keys( icons ).map( group => {
			...
			Object.keys( groupIcons ).map( iconName => {
				iconName = groupIcons[ iconName ];
				iconOptions.push({
					value: `${ prefix } fa-${ iconName }`,
					label: `${ iconName }`
				});
			});
		});
		...
		return (
			<BaseControl label={ label } id={ id } help={ help } className={ className }>
				<div className="components-mb-icon-control__input">
					<Select
